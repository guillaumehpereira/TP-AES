...
public class AES {
	...
	public AES(Block key) {
		// Initialiser les sous-clés
		Key initialKey = new Key(key);
		this.keys = initialKey.genSubKeys(new SBox(s));

		// Initialiser les S-boxes
		this.sbox = new SBox(s);
		this.sboxInv = new SBox(sInv);

		// Initialiser les matrices de mixage
	...
	public Block cypher(Block plaintext) {
		...
		// Étape 1: AddRoundKey
		state = state.xOr(keys[0]);

		for (int round = 1; round < 10; round++) {
			// Étape 2: SubstituteBytes
		...
		// Dernière étape sans MixColumns
		state = state.substitute(sbox);
		state = state.shift();
		state = state.xOr(keys[10]);
	...
	public Block deCypher(Block cyphertext) {
		...
		for (int round = 9; round > 0; round--) {
			// Étape 2: InvShiftRows
			state = state.shiftInv();

			// Étape 3: InvSubstituteBytes
			state = state.substitute(sboxInv);

			// Étape 4: AddRoundKey
			state = state.xOr(keys[round]);

			// Étape 5: InvMixColumns
			state = state.mult(mixStateInv);
		}

		// Dernière étape sans InvMixColumns
		state = state.shiftInv();
		state = state.substitute(sboxInv);
		state = state.xOr(keys[0]);
